...
import           Text.Megaparsec.Char (alphaNumChar, char, hexDigitChar, numberChar, string, eol, lowerChar)
...
parser = passport `sepEndBy1` eol where
    passport = Map.fromList <$> (field `sepEndBy1` (eol <|> string " "))
    field = do
...
checkers =
    ...
    ,   ("hgt", parseAndCheck (Cm <$> try (decimal <* string "cm") <|> In <$> decimal <* string "in")
                            \case
                                Cm h -> h >= 150 && h <= 193
                                In h -> h >= 59 && h <= 76
        )
