...
import           Data.List (last, lines, unlines, stripPrefix)
...
replaceWith _ _ [] = []
replaceWith a b xs | Just ys <- stripPrefix a xs = b ++ replaceWith a b ys
...
solve' pgm = do
    ...
    let p1 = sum [product p | p <- tiles, all (\p' -> (grid' !? toIx2 p') == Just '#') (adjacent p)]
